...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const isOutOfStock = (Number(product.available ?? product.stock) || 0) <= 0;
    ...
    <Link href={`/shop/products/${product.id}`} className="group card hover:shadow-md transition-all duration-300">
      <div className="relative aspect-[3/4] overflow-hidden" style={{ backgroundColor: 'var(--color-surface-alt)' }}>
        ...
        <button
          ...
          className={`absolute top-3 right-3 z-10 w-10 h-10 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-200 shadow-md ${
            isFavorite
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-1 max-w-[60%]">
          {product.is_on_sale && (
            <span className="bg-red-600 text-white text-xs font-medium px-2 py-0.5 rounded-full">Oferta</span>
          )}
      ...
      <div className="p-4">
        <h3 className="font-semibold text-[var(--color-text)] text-sm leading-tight line-clamp-2 group-hover:text-blush-600 transition-colors">
          {product.name}
        </h3>
        <div className="flex items-center justify-between mt-3">
          <div className="flex flex-col">
            {product.is_on_sale && product.sale_price ? (
              <>
                <span className="text-red-600 dark:text-red-400 font-bold text-lg">${Number(product.sale_price).toFixed(2)}</span>
                <span className="text-[var(--color-text-muted)] text-xs line-through">${Number(product.price).toFixed(2)}</span>
              </>
            ) : (
